...
function requireDatabase(req, res, next) {
  const db = getPool();
  if (!db) return res.status(503).json({ error: 'Database is not configured.', action: 'Add DATABASE_URL in Vercel environment variables.' });
  req.db = db;
  next();
}
...
async function getProfile(db, userId) {
  const { rows } = await db.query(`
    SELECT u.id,u.username,u.email,u.display_name,u.avatar_seed,u.created_at,
           p.preferred_language,p.theme,p.notify_goals,p.notify_kickoff,p.notify_halftime,
           p.notify_fulltime,p.notify_red_cards,p.notify_news
    FROM users u LEFT JOIN user_preferences p ON p.user_id=u.id WHERE u.id=$1
  `, [userId]);
  if (!rows.length) return null;
  const r = rows[0];
  return {
    user: { id:r.id, username:r.username, email:r.email, display_name:r.display_name, avatar_seed:r.avatar_seed, created_at:r.created_at },
    preferences: {
      preferred_language:r.preferred_language || 'en', theme:r.theme || 'system',
      notify_goals:r.notify_goals ?? true, notify_kickoff:r.notify_kickoff ?? true,
      notify_halftime:r.notify_halftime ?? false, notify_fulltime:r.notify_fulltime ?? true,
      notify_red_cards:r.notify_red_cards ?? true, notify_news:r.notify_news ?? false
    }
  };
}
...
app.get('/api/health', asyncRoute(async (req, res) => {
  const db = getPool();
  if (!db) return res.status(503).json({ ok:false, service:'EthioLiveScores API', database:'not-configured', push:pushEnabled ? 'configured' : 'not-configured' });
  await db.query('SELECT 1');
  res.json({ ok:true, service:'EthioLiveScores API', database:'connected', push:pushEnabled ? 'configured' : 'not-configured' });
}));

app.get('/api/matches', requireDatabase, asyncRoute(async (req, res) => {
  const { rows } = await req.db.query(`
    SELECT m.*,l.name_en AS league_name,l.name_am AS league_name_am,
           t1.name_en AS home_en,t1.name_am AS home_am,t1.short_name AS home_short,
           t2.name_en AS away_en,t2.name_am AS away_am,t2.short_name AS away_short
    FROM matches m JOIN leagues l ON l.id=m.league_id
    JOIN teams t1 ON m.home_team_id=t1.id JOIN teams t2 ON m.away_team_id=t2.id
...
app.get('/api/teams', requireDatabase, asyncRoute(async (req, res) => {
  const { rows } = await req.db.query('SELECT id,name_en,name_am,short_name,avatar_seed FROM teams ORDER BY name_en');
  res.json(rows);
}));

app.get('/api/competitions', requireDatabase, asyncRoute(async (req, res) => {
  if (await tableExists(req.db, 'competitions')) {
    const { rows } = await req.db.query(`SELECT id,slug,name_en,name_am,category,season,team_count,accent FROM competitions WHERE is_active=true ORDER BY sort_order,name_en`);
    return res.json(rows);
  }
  const { rows } = await req.db.query(`SELECT id,LOWER(REGEXP_REPLACE(name_en,'[^a-zA-Z0-9]+','-','g')) AS slug,name_en,name_am,'Domestic' AS category,NULL::text AS season,0 AS team_count,'#0B46A8' AS accent FROM leagues ORDER BY name_en`);
  res.json(rows);
}));

app.get('/api/competitions/:slug', requireDatabase, asyncRoute(async (req, res) => {
  const slug = String(req.params.slug || '').toLowerCase();
  let competition = null;
  if (await tableExists(req.db, 'competitions')) {
    const found = await req.db.query(`SELECT id,slug,name_en,name_am,category,season,team_count,accent FROM competitions WHERE slug=$1 AND is_active=true LIMIT 1`, [slug]);
    competition = found.rows[0] || null;
  }
  if (!competition) {
    const found = await req.db.query(`SELECT id,LOWER(REGEXP_REPLACE(name_en,'[^a-zA-Z0-9]+','-','g')) AS slug,name_en,name_am,'Domestic' AS category,NULL::text AS season,0 AS team_count,'#0B46A8' AS accent FROM leagues WHERE LOWER(REGEXP_REPLACE(name_en,'[^a-zA-Z0-9]+','-','g'))=$1 LIMIT 1`, [slug]);
    competition = found.rows[0] || null;
  }
  if (!competition) return res.status(404).json({ error:'Competition is not in the live database yet.', slug });

  const league = await req.db.query('SELECT id FROM leagues WHERE LOWER(name_en)=LOWER($1) LIMIT 1', [competition.name_en]);
  let matches = [];
  if (league.rows.length) {
    const result = await req.db.query(`
      SELECT m.*,l.name_en AS league_name,l.name_am AS league_name_am,
             h.name_en AS home_en,h.name_am AS home_am,a.name_en AS away_en,a.name_am AS away_am
      FROM matches m JOIN leagues l ON l.id=m.league_id
      JOIN teams h ON h.id=m.home_team_id JOIN teams a ON a.id=m.away_team_id
      WHERE m.league_id=$1 ORDER BY m.match_date DESC LIMIT 50
    `, [league.rows[0].id]);
    matches = result.rows;
  }
  const standings = slug === 'ethiopian-premier-league' ? (await req.db.query('SELECT * FROM league_standings LIMIT 30')).rows : [];
  res.json({
    competition: {
      slug:competition.slug, code:String(competition.name_en).split(/\s+/).map(x=>x[0]).join('').slice(0,5).toUpperCase(),
      name:competition.name_en, nameAm:competition.name_am, scope:competition.category === 'CAF' ? 'Africa' : 'Ethiopia',
      type:competition.category || 'Competition', description:'Live competition data from EthioLiveScores.', descriptionAm:'ከEthioLiveScores የቀጥታ የውድድር መረጃ።'
    },
    matches, standings, demo:false
  });
}));

app.get('/api/news', requireDatabase, asyncRoute(async (req, res) => {
  if (!(await tableExists(req.db, 'news_articles'))) return res.json([]);
  const { rows } = await req.db.query(`
    SELECT n.slug,n.category,c.slug AS "competitionSlug",n.title_en AS "titleEn",n.title_am AS "titleAm",
           n.summary_en AS "summaryEn",n.summary_am AS "summaryAm",COALESCE(n.body_en,'') AS "bodyEn",
           COALESCE(n.body_am,'') AS "bodyAm",n.published_at AS "publishedAt",false AS demo
    FROM news_articles n LEFT JOIN competitions c ON c.id=n.competition_id
    WHERE n.status='published' AND n.published_at<=NOW() ORDER BY n.published_at DESC LIMIT 60
  `);
  res.json(rows);
}));
app.get('/api/news/:slug', requireDatabase, asyncRoute(async (req, res) => {
  if (!(await tableExists(req.db, 'news_articles'))) return res.status(404).json({ error:'Article not found.' });
  const { rows } = await req.db.query(`
    SELECT n.slug,n.category,c.slug AS "competitionSlug",n.title_en AS "titleEn",n.title_am AS "titleAm",
           n.summary_en AS "summaryEn",n.summary_am AS "summaryAm",COALESCE(n.body_en,'') AS "bodyEn",
           COALESCE(n.body_am,'') AS "bodyAm",n.published_at AS "publishedAt",false AS demo
    FROM news_articles n LEFT JOIN competitions c ON c.id=n.competition_id
    WHERE n.slug=$1 AND n.status='published' AND n.published_at<=NOW() LIMIT 1
  `, [req.params.slug]);
...
app.post('/api/auth/register', requireDatabase, asyncRoute(async (req, res) => {
  ...
  const hash=await bcrypt.hash(password,12), seed=username.toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'')||'fan', client=await req.db.connect();
  try {
    await client.query('BEGIN');
    const { rows }=await client.query(`INSERT INTO users(username,email,password_hash,display_name,avatar_seed) VALUES($1,$2,$3,$1,$4) RETURNING id`,[username,email,hash,seed]);
    await client.query('INSERT INTO user_preferences(user_id) VALUES($1) ON CONFLICT(user_id) DO NOTHING',[rows[0].id]);
    await client.query('COMMIT');
    const profile=await getProfile(req.db,rows[0].id);
    res.status(201).json({token:signUser(rows[0].id),...profile});
  } catch(error) {
...
app.post('/api/auth/login', requireDatabase, asyncRoute(async (req, res) => {
  ...
  const { rows }=await req.db.query('SELECT id,password_hash FROM users WHERE email=$1',[email]);
  if(!rows.length||!(await bcrypt.compare(password,rows[0].password_hash))) return res.status(400).json({error:'Invalid email or password.'});
  const profile=await getProfile(req.db,rows[0].id);
  res.json({token:signUser(rows[0].id),...profile});
}));
app.get('/api/auth/me', verifyToken, requireDatabase, asyncRoute(async (req,res)=>{
  const profile=await getProfile(req.db,req.user.userId);
  if(!profile) return res.status(404).json({error:'Account not found.'});
  res.json(profile);
}));
app.patch('/api/auth/me', verifyToken, requireDatabase, asyncRoute(async (req,res)=>{
  const username=req.body?.username===undefined?null:String(req.body.username).trim(), displayName=req.body?.display_name===undefined?null:String(req.body.display_name).trim(), avatarSeed=req.body?.avatar_seed===undefined?null:String(req.body.avatar_seed).trim();
  if(username!==null&&(username.length<3||username.length>50)) return res.status(400).json({error:'Username must be 3–50 characters.'});
  if(displayName!==null&&displayName.length>80) return res.status(400).json({error:'Display name is too long.'});
  if(avatarSeed!==null&&avatarSeed.length>100) return res.status(400).json({error:'Avatar seed is too long.'});
  try { await req.db.query(`UPDATE users SET username=COALESCE($2,username),display_name=COALESCE($3,display_name),avatar_seed=COALESCE($4,avatar_seed) WHERE id=$1`,[req.user.userId,username,displayName,avatarSeed]); }
  catch(error){ if(error.code==='23505') return res.status(409).json({error:'That username is already taken.'}); throw error; }
  res.json(await getProfile(req.db,req.user.userId));
}));
...
app.post('/api/push/subscribe',verifyToken,requireDatabase,asyncRoute(async(req,res)=>{
  if (!(await tableExists(req.db,'push_subscriptions'))) return res.status(503).json({error:'Push subscription table is not installed. Run the database migrations.'});
...
app.post('/api/internal/push/event',requireDatabase,asyncRoute(async(req,res)=>{
  if(!PUSH_DISPATCH_SECRET||req.header('x-push-secret')!==PUSH_DISPATCH_SECRET)return res.status(403).json({error:'Invalid dispatch secret.'});
  if(!pushEnabled)return res.status(503).json({error:'Push delivery is not configured.'});
  if(!(await tableExists(req.db,'push_subscriptions')))return res.status(503).json({error:'Push subscription table is not installed.'});
  const type=String(req.body?.type||''),column=preferenceColumn(type);if(!column)return res.status(400).json({error:'Unsupported notification type.'});
  const teamId=Number(req.body?.teamId||0),params=[];let where=`p.${column}=true`;
  if(teamId){params.push(teamId);where+=` AND EXISTS(SELECT 1 FROM user_favorite_teams f WHERE f.user_id=u.id AND f.team_id=$1)`}
  const users=await req.db.query(`SELECT u.id FROM users u JOIN user_preferences p ON p.user_id=u.id WHERE ${where}`,params);
  const payload={title:String(req.body?.title||'EthioLiveScores').slice(0,180),body:String(req.body?.body||'New football update').slice(0,500),url:String(req.body?.url||'/'),tag:String(req.body?.tag||type),type};
  const results=await Promise.all(users.rows.map(row=>deliverPush(req.db,row.id,payload)));
  res.json({ok:true,users:users.rowCount,sent:results.reduce((a,r)=>a+(r.sent||0),0),failed:results.reduce((a,r)=>a+(r.failed||0),0)});
}));

const distDir=path.join(__dirname,'dist');
if(fs.existsSync(distDir))app.use(express.static(distDir,{index:false,maxAge:'1h'}));
