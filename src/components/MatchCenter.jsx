...
export default function MatchCenter({onNeedAuth}){
  ...
  const loadData=async(silent=false,background=false)=>{if(!silent)setLoading(true);else if(!background)setRefreshing(true);try{const[m,s,p]=await Promise.allSettled([apiFetch(`/matches?date=${selectedDate}`),apiFetch('/standings'),apiFetch('/provider/status')]);const matchData=m.status==='fulfilled'?m.value:[];const table=s.status==='fulfilled'?s.value:[];setMatches(matchData?.length?matchData:demoMatches.filter(x=>isoDate(x.match_date)===selectedDate));setStandings(table?.length?table:demoStandings);setProvider(p.status==='fulfilled'?p.value:null);setDemo(!(matchData?.length));}catch{setMatches(demoMatches.filter(x=>isoDate(x.match_date)===selectedDate));setStandings(demoStandings);setDemo(true)}finally{setLoading(false);if(!background)setRefreshing(false)}};
  useEffect(()=>{loadData();},[selectedDate]);
  useEffect(()=>{const id=setInterval(()=>{if(document.visibilityState!=='visible')return;if(Date.now()-activityRef.current<1800)return;loadData(true,true)},provider?.configured?15000:30000);return()=>clearInterval(id)},[selectedDate,provider?.configured]);
  ...
  return <main className="page score-swipe-surface" id="scores" onTouchStart={onTouchStart} onTouchEnd={onTouchEnd}>
    ...
    <div className="content-grid"><section className="match-column" id="fixtures">
      {loading?<div className="league-card loading-card"><div className="loading-shimmer"/><div className="loading-shimmer short"/><div className="loading-shimmer"/></div>:Object.keys(groups).length?Object.entries(groups).map(([league,list],groupIndex)=><section className="league-card" key={league}>
        <div className="league-head"><div className="league-identity"><span className="league-logo">{list[0]?.league_logo?<img src={list[0].league_logo} alt={`${league} logo`}/>:<Shield size={18}/>}</span><div><b>{league}</b><small>{list[0]?.country||'Ethiopia'}</small></div></div><button onClick={()=>navigate(`/competition/${leagueToSlug(league)}`)}>View league <ChevronRight size={14}/></button></div>
    ...
    <aside className="side-column">
      <section className="side-card" id="standings"><div className="side-head"><b>League table</b><small>TOP 5</small></div><div className="table-head"><span>#</span><span>TEAM</span><span>MP</span><span>PTS</span></div>{(standings.length?standings:demoStandings).slice(0,5).map((r,i)=>{const logo=verifiedLogo(r.name_en,r.logo);return <div className="standing-row" key={r.team_id||r.name_en}><span>{r.rank||i+1}</span><span>{logo?<img className="tiny-logo-img" src={logo} alt={`${r.name_en} crest`} referrerPolicy="no-referrer"/>:<span className="tiny-team"><Shield size={14}/></span>}{r.name_en}</span><span>{r.mp}</span><b>{r.pts}</b></div>})}</section>
